...
# Training scripts serialized these infosets while running as __main__.
# Register aliases so joblib can unpickle them from the GUI entrypoint too.
__main__.PreflopHoldemInfoSet = PreflopHoldemInfoSet
__main__.PostflopHoldemInfoSet = PostflopHoldemInfoSet
...
class AIPlayer(Player):
    def __init__(self, balance) -> None:
        ...
        if pyttsx3 is None:
            print("Text-to-speech disabled: pyttsx3 is not available")
            self.speak = False
            self.engine = SilentSpeechEngine()
        else:
            try:
                self.engine = pyttsx3.init()
            except Exception as exc:
                print(f"Text-to-speech disabled: {exc}")
                self.speak = False
                self.engine = SilentSpeechEngine()
