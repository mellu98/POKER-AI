...
USE_KMEANS = False  # Runtime infosets in this repo are keyed for fast 10-bucket equity clusters.
...
def _load_latest_centroids(stage):
    filename = sorted(get_filenames(f"../kmeans_data/centroids/{stage}"))[-1]
    print(f"Loading KMeans {stage.title()} centroids", filename)
    return np.load(f"../kmeans_data/centroids/{stage}/{filename}")
...
def load_kmeans_classifiers():
    ...
    if KMeans is None:
        kmeans_flop = _load_latest_centroids("flop")
        kmeans_turn = _load_latest_centroids("turn")
    else:
    ...
    assert len(kmeans_flop.cluster_centers_ if hasattr(kmeans_flop, "cluster_centers_") else kmeans_flop) == NUM_FLOP_CLUSTERS
    assert len(kmeans_turn.cluster_centers_ if hasattr(kmeans_turn, "cluster_centers_") else kmeans_turn) == NUM_TURN_CLUSTERS
...
if USE_KMEANS:
    ...
    try:
        ...
        print(e, "Couldn't load KMeans classifiers or centroids. Falling back to fast equity clusters.")
        USE_KMEANS = False
        NUM_FLOP_CLUSTERS = 10
        NUM_TURN_CLUSTERS = 10
...
def predict_cluster_kmeans(kmeans_classifier, cards, n=200):
    ...
    if hasattr(kmeans_classifier, "predict"):
        y = kmeans_classifier.predict([equity_distribution])
        assert len(y) == 1
        return y[0]

    centroids = np.asarray(kmeans_classifier)
    distances = np.linalg.norm(centroids - np.asarray(equity_distribution), axis=1)
    ...
    if clustering:
        ...
        for stage in stages:
            ...
            if not os.path.exists(f"../kmeans_data/centroids/{stage}/centroids_{filename}"):
                if KMeans is None:
                    raise RuntimeError("scikit-learn is required to generate KMeans classifiers")
                if stage == "flop":
