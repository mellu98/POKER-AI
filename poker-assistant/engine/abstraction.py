...
# Lazy imports for heavy training-only dependencies
# (matplotlib, sklearn, tqdm are only needed for dataset generation / plotting)
...
# Absolute base path so imports work regardless of working directory
_KMEANS_BASE = Path(__file__).parent.parent / "kmeans_data" / "kmeans"
...
def load_kmeans_classifiers():
    ...
    flop_dir = _KMEANS_BASE / "flop"
    filename = sorted(get_filenames(str(flop_dir)))[-1]
    print("Loading KMeans Flop Classifier", filename)
    kmeans_flop = joblib.load(str(flop_dir / filename))

    turn_dir = _KMEANS_BASE / "turn"
    filename = sorted(get_filenames(str(turn_dir)))[-1]
    print("Loading KMeans Turn Classifier", filename)
    kmeans_turn = joblib.load(str(turn_dir / filename))
...
def calculate_equity(player_cards: List[str], community_cards=[], n=2000, timer=False):
    ...
    # Normalizza e valida le carte
    player_cards = [_norm_card(c) for c in player_cards]
    community_cards = [_norm_card(c) for c in community_cards]
    all_known = player_cards + community_cards
    all_known = [c for c in all_known if c and len(c) == 2]

    if len(set(all_known)) != len(all_known):
        dupes = [c for c in set(all_known) if all_known.count(c) > 1]
        print(f"[calculate_equity] DUPLICATE CARDS DETECTED: {dupes} in {all_known}")
        return 0.5

    deck = fast_evaluator.Deck(excluded_cards=all_known)
    ...
    for _ in range(n):
        ...
        try:
            ...
            print(f"[calculate_equity] phevaluator error with known {all_known} + opp {opponent_cards}: {e}")
            return 0.5
