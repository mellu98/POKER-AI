"""Three preflop CFR brains (base / vs-nit / vs-maniac), routed by a K-Means opponent classifier."""
...
CFR_STRATEGY = None       # base GTO strategy (fallback)
CFR_VS_NIT = None         # exploit strategy vs nits
CFR_VS_MANIAC = None      # exploit strategy vs maniacs
BUCKET_IDX = None
...
try:
    ...
    # Load all CFR brains: base + exploits
    for _name, _global in [("cfr_base.npz", "CFR_STRATEGY"),
                            ("cfr_vs_nit.npz", "CFR_VS_NIT"),
                            ("cfr_vs_maniac.npz", "CFR_VS_MANIAC"),
                            ("preflop_cfr_strategy.npz", None)]:  # v2 fallback name
        _p = os.path.join(DATA_DIR, _name)
        if os.path.exists(_p):
            _loaded = np.load(_p)
            if _global:
                globals()[_global] = _loaded
            elif CFR_STRATEGY is None:  # only use old name if cfr_base wasn't found
                CFR_STRATEGY = _loaded
...
def _pick_cfr_brain(aggro_tag):
    """Route to the correct CFR brain based on opponent archetype."""
    if aggro_tag == "nit" and CFR_VS_NIT is not None:
        return CFR_VS_NIT
    if aggro_tag == "maniac" and CFR_VS_MANIAC is not None:
        return CFR_VS_MANIAC
    return CFR_STRATEGY  # base GTO (may also be None)

def sample_cfr_action(state, cfr_key, cfr_source=None):
    """Uses a random number to sample from CFR frequencies."""
    source = cfr_source or CFR_STRATEGY
    if source is None or cfr_key not in source: return None
    
    cards = state["your_cards"]
    bucket_idx = BUCKET_IDX.get(_hand_bucket(cards[0], cards[1]))
    if bucket_idx is None: return None
    
    strat = source[cfr_key][bucket_idx]
...
def preflop_decision(state, aggro_tag, analysis, opp_models, active_opps):
    ...
    # --- CFR INJECTION: route to exploit brain based on opponent type ---
    cfr_key = get_cfr_key(state, analysis)
    if cfr_key:
        cfr_brain = _pick_cfr_brain(aggro_tag)
        cfr_action = sample_cfr_action(state, cfr_key, cfr_source=cfr_brain)
        if cfr_action:
            return cfr_action
