"""Preflop CFR brain plus live per-seat opponent profiling (VPIP/aggression) that drives
exploit adjustments: steal-widen vs confirmed nits, tighten vs maniacs, inelastic value
sizing vs calling stations, board-texture c-bet bluffs, dynamic variance buffer and bust guard."""
...
# ---------------------------------------------------------------------------
# Persistent Opponent Profiling
# ---------------------------------------------------------------------------
# Accumulates into module-level globals so data survives the 200-entry
# match_action_log truncation. Falls back to re-parsing when globals
# are empty (first call of a new match).

_OPP_STATS: dict = {}      # seat -> raw counters
_SEEN_HANDS: set = set()
_LAST_HAND_NUM = -1


def _reset_opp_globals():
    global _OPP_STATS, _SEEN_HANDS, _LAST_HAND_NUM
    _OPP_STATS, _SEEN_HANDS, _LAST_HAND_NUM = {}, set(), -1


def _ensure_opp(seat):
    s = _OPP_STATS.get(seat)
    if s is None:
        s = {"hands": 0, "vpip": 0, "pfr": 0, "aggr": 0, "passive": 0}
        _OPP_STATS[seat] = s
    return s


def _ingest_hand(actions, my_seat):
    """Process one hand's actions into persistent stats."""
    seen, vpip_done, pfr_done = set(), set(), set()
    for a in actions:
        ...
        if seat not in seen:
            seen.add(seat)
            _ensure_opp(seat)["hands"] += 1
        if act in ("call", "raise", "all_in") and seat not in vpip_done:
            vpip_done.add(seat)
            _ensure_opp(seat)["vpip"] += 1
        if act in ("raise", "all_in") and seat not in pfr_done:
            pfr_done.add(seat)
            _ensure_opp(seat)["pfr"] += 1
        if act in ("raise", "all_in"):
            _ensure_opp(seat)["aggr"] += 1
        elif act == "call":
            _ensure_opp(seat)["passive"] += 1


def _update_persistent_model(match_log, my_seat):
    """Incrementally ingest only new hands from match_action_log."""
    global _LAST_HAND_NUM
    by_hand = {}
    for a in match_log:
        h = a.get("hand_num")
        if h is not None:
            by_hand.setdefault(h, []).append(a)
    # Detect match reset (hand 0 reappears after we've seen later hands)
    if 0 in by_hand and _LAST_HAND_NUM > 5:
        _reset_opp_globals()
    for h in sorted(by_hand):
        if h not in _SEEN_HANDS:
            _ingest_hand(by_hand[h], my_seat)
            _SEEN_HANDS.add(h)
            if h > _LAST_HAND_NUM:
                _LAST_HAND_NUM = h
...
def model_opponents(match_log, my_seat):
    """Returns the same dict format as before, but backed by persistent globals."""
    _update_persistent_model(match_log, my_seat)
    result = {}
    for seat, s in _OPP_STATS.items():
        if seat == my_seat:
            continue
        h = max(s["hands"], 1)
        tot = s["aggr"] + s["passive"]
        result[seat] = {
            "vpip": s["vpip"] / h,
            "agg_freq": s["aggr"] / max(tot, 1),
            "hands_seen": s["hands"],
        }
    return result
...
# ---------------------------------------------------------------------------
# Variance Buffer (time + stack + risk scaled)
# ---------------------------------------------------------------------------

def variance_buffer(stack, owed, hands_left):
    """Dynamic call threshold: tighter when risking tournament life early,
    looser when ahead late or when the pot is small relative to stack."""
    if stack <= 0:
        return 0.04
    sf = owed / stack
    base = 0.04
    rm = (2.8 if sf >= 0.90 else
          2.0 if sf >= 0.50 else
          1.4 if sf >= 0.30 else 1.0)
    tm = 1.0 + hands_left / MATCH_HANDS
    dm = (1.15 if stack >= STARTING_STACK * 1.5 else
          1.0  if stack >= STARTING_STACK else
          0.9  if stack >= STARTING_STACK * 0.5 else 0.75)
    return max(0.02, min(0.12, base * rm * tm * dm))


def remaining_hands(state):
    ml = state.get("match_action_log") or []
    cur = max((a.get("hand_num", -1) for a in ml), default=0)
    return max(0, MATCH_HANDS - max(cur, 0) - 1)
...
def preflop_decision(state, aggro_tag, analysis, opp_models, active_opps):
    ...
    if owed >= stack * 0.85:
        required = (owed / max(pot + owed, 1)) + 0.10
...
def postflop_decision(state, aggro_tag, analysis, deadline, active_opps, opp_models, hands_left):
    pot, owed, stack, my_bet = state["pot"], state["amount_owed"], state["your_stack"], state["your_bet_this_street"]
    ...
    if not state["can_check"]:
        # PATCHED: dynamic variance buffer instead of flat 0.04
        buffer = variance_buffer(stack, owed, hands_left)
        
        # Per-opponent buffer scaling
        if n_opps == 1 and active_opps:
            s = active_opps[0]["seat"]
            if s in opp_models and opp_models[s]["hands_seen"] >= 10:
                if aggro_tag == "maniac": buffer *= 0.6
                elif aggro_tag == "nit": buffer *= 1.3
        
        required = (owed / max(pot + owed, 1)) + buffer + (n_opps - 1) * 0.015
        ...
        # PATCHED: bust-prevention guard
        if eq >= required:
            guard = 0.04 if aggro_tag == "maniac" else 0.07
            if owed >= stack * 0.70 and eq < required + guard:
                return {"action": "fold"}
            return {"action": "call"}
...
def decide(state: dict) -> dict:
    ...
    try:
        ...
        return postflop_decision(state, aggro_tag, analysis, time.time() + 1.5, active_opps, opp_models, remaining_hands(state))
    except Exception:
        return _fold_or_check(state)